            ...
            if (player != null) {
                ...
                if (configSettings.radius != -1 || configSettings.botArea.contains(player)) {
                    ...
                    if (player.getAnimationId() == -1 || player.isMoving()) {
                        ...
                        GameObject gameObject = GameObjects.newQuery().names(configSettings.interactableItems.getInteractionNames()).within(configSettings.botArea).results().nearest();
                        Npc npc;
                        boolean isGameObject = false;
                        LocatableEntity locatableEntity = null;
                        int interactableItemID = -1;
                        if (gameObject != null && gameObject.getDefinition() != null) {
                            locatableEntity = gameObject;
                            interactableItemID = configSettings.interactableItems.getIndexByInteractionName(gameObject.getDefinition().getName());
                            isGameObject = true;
                        } else if ((npc = Npcs.newQuery().names(configSettings.interactableItems.getInteractionNames()).within(configSettings.botArea).results().nearest()) != null && npc.getDefinition() != null) {
                            locatableEntity = npc;
                            interactableItemID = configSettings.interactableItems.getIndexByInteractionName(npc.getName());
                            isGameObject = false;
                        }
                        if (interactableItemID != -1) {
                            globals.currentAction = configSettings.interactableItems.getActionings()[interactableItemID] + ' ' + configSettings.interactableItems.getNames()[interactableItemID];
                            if (locatableEntity.isVisible()) {
                                if (locatableEntity.interact(configSettings.interactableItems.getActionNames()[interactableItemID], configSettings.interactableItems.getInteractionNames()[interactableItemID])) {
                                ...
                                globals.currentAction = "Turing to face " + configSettings.interactableItems.getInteractionNames()[interactableItemID];
                                Camera.turnTo(locatableEntity);
                                if (!locatableEntity.isVisible()) {
                                    globals.currentAction = "Going to " + configSettings.interactableItems.getInteractionNames()[interactableItemID];
                                    ViewportPath p;
